...
export default function AmostrasCreationForm({ open, onOpenChange, clienteNome, representanteNome, codigoCliente, formData, onFormDataChange, onConfirm }: Props) {
        ...
        {products.map(p => {
          ...
          return (
            <div key={key} className="flex items-center gap-1.5 text-xs">
              ...
              />
              <label htmlFor={key} className="cursor-pointer flex-1 truncate">
  ...
  const generatePdf = (): Blob => {
    ...
    // Logo
    try {
      // Logo will be added via image load in async version
    } catch { }
    ...
    addRow2('CLIENTE:', clienteNome, 'CÓDIGO:', codigoCliente);
